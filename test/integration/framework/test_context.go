...
func registerFlags(t *TestContextType) {
	...
	flag.BoolVar(&t.IsBlockingTest, "blocking-tests", true, "If set, run the tests that cannot be run concurrently between multiple builds.")
	flag.StringVar(&t.WiremockDataRootDir, "wiremock-repo-dir", "",
		`Path to kubeturbo-wiremock repo. By default it will assume it's in the same directory as kubeturbo
		 and that you're running the integration tests through ./build/integration.test.`)
}
