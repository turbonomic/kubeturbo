...
func (s *VMTServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&s.UpdateQuotaToAllowMoves, "update-quota-to-allow-moves", true, "Indicate if kubeturbo tries to update namespace quotas to allow pod moves when quota(s) is/are full. Default is set to true.")
