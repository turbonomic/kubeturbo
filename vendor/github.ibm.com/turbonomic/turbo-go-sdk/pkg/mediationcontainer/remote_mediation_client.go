...
func CreateRemoteMediationClient(allProbes map[string]*ProbeProperties,
	...
	remoteMediationClient.createMessageHandlers()
...
func (remoteMediationClient *remoteMediationClient) Init(probeRegisteredMsgCh chan bool, disconnectFromTurbo chan struct{}, refreshTokenChannel chan struct{}, jwTokenChannel chan string) {
	...
	glog.V(3).Infof("Initialize remote mediation client - create connection configuration")
	...
	sdkProtocolHandler := CreateSdkClientProtocolHandler(remoteMediationClient.allProbes,
		...
		remoteMediationClient.containerConfig.ClientId,
	...
	glog.V(3).Infof("Initialize remote mediation client - client websocket transport connected")
	...
	go func() {
		...
		for {
			...
			case err := <-transport.NotifyReconnect():
				glog.Infof("[Reconnect] resetting weksocket connection due to error: %v", err)
				// closing and resetting the connection will trigger a reconnect via the logic above (case <-transport.NotifyClosed())
				transport.closeAndResetWebSocket()
...
const (
	...
	ACTION_LIST_REQUEST  RequestType = "ActionList"
...
func getRequestType(serverRequest *proto.MediationServerMessage) RequestType {
	...
	} else if serverRequest.GetActionListRequest() != nil {
		return ACTION_LIST_REQUEST
	} else {
		return UNKNOWN_REQUEST
	}
}

type DiscoveryRequestConfigUpdater interface {
	UpdateDiscoveryConfiguration(chunkSendDelayMillis int, numObjectsPerChunk int)
}
...
func (remoteMediationClient *remoteMediationClient) createMessageHandlers() {
	...
	remoteMediationClient.MessageHandlers[DISCOVERY_REQUEST] = &DiscoveryRequestHandler{
		...
		discoveryResponseSender: DiscoveryResponseSender{
			chunkSendDelayMillis: remoteMediationClient.containerConfig.ChunkSendDelayMillis,
			numObjectsPerChunk:   remoteMediationClient.containerConfig.NumObjectsPerChunk,
		},
	}
	...
	remoteMediationClient.MessageHandlers[ACTION_LIST_REQUEST] = &ActionListMessageHandler{
		probes: allProbes,
	}
...
func (discReqHandler *DiscoveryRequestHandler) UpdateDiscoveryConfiguration(chunkSendDelayMillis int, numObjectsPerChunk int) {
	discReqHandler.discoveryResponseSender.chunkSendDelayMillis = chunkSendDelayMillis
	discReqHandler.discoveryResponseSender.numObjectsPerChunk = numObjectsPerChunk
}
...
// -------------------------------- Action List Request Handler -----------------------------------
// Message handler that will receive the Action List Request for entities in the TurboProbe.
// Action list request will be delegated to the right TurboProbe. Multiple ActionProgress and final ActionResult
// responses are sent back to the server.
type ActionListMessageHandler struct {
	probes map[string]*ProbeProperties
}

func (actionReqHandler *ActionListMessageHandler) HandleMessage(serverRequest *proto.MediationServerMessage,
	probeMsgChan chan *proto.MediationClientMessage,
) {
	glog.V(4).Infof("[ActionListMessageHandler] Received: action list request %s", serverRequest)
	request := serverRequest.GetActionListRequest()
	probeType := request.ProbeType
	if actionReqHandler.probes[*probeType] == nil {
		glog.Errorf("Received: action list request for unknown probe type : %s", *probeType)
		return
	}
	for idx, single := range request.ActionExecution {
		glog.V(3).Infof("Received[%d]: action list request for probe: %s and action: %s with %d action items ", idx,
			*probeType, single.ActionType, len(single.ActionItem))
	}
	probeProps := actionReqHandler.probes[*probeType]
	turboProbe := probeProps.Probe

	msgID := serverRequest.GetMessageID()
	worker := NewActionListResponseWorker(msgID, turboProbe,
		request.ActionExecution, request.GetAccountValue(), probeMsgChan)
	worker.start()
}
...
type ActionListResponseWorker struct {
	msgId               int32
	turboProbe          *probe.TurboProbe
	actionExecutionDtos []*proto.ActionExecutionDTO
	accountValues       []*proto.AccountValue
	probeMsgChan        chan *proto.MediationClientMessage
}

func NewActionListResponseWorker(msgId int32, turboProbe *probe.TurboProbe,
	actionExecutionDtos []*proto.ActionExecutionDTO, accountValues []*proto.AccountValue,
	probeMsgChan chan *proto.MediationClientMessage,
) *ActionListResponseWorker {
	worker := &ActionListResponseWorker{
		msgId:               msgId,
		turboProbe:          turboProbe,
		actionExecutionDtos: actionExecutionDtos,
		accountValues:       accountValues,
		probeMsgChan:        probeMsgChan,
	}
	var actionTypes []string
	for _, single := range actionExecutionDtos {
		actionTypes = append(actionTypes, single.ActionType.String())
	}
	glog.V(3).Infof("New ActionListResponseProtocolWorker for %d %+v %s", msgId, turboProbe,
		actionTypes)
	return worker
}

func (actionListWorker *ActionListResponseWorker) start() {
	// Execute the action
	actionListResult := actionListWorker.turboProbe.ExecuteActionList(actionListWorker.actionExecutionDtos, actionListWorker.accountValues, actionListWorker)
	clientMsg := NewClientMessageBuilder(actionListWorker.msgId).SetActionListResponse(actionListResult).Create()

	// Send the response on the callback channel to send to the server
	actionListWorker.probeMsgChan <- clientMsg // This will block till the channel is ready to receive
	glog.V(3).Infof("Sent action list response for %d.", clientMsg.GetMessageID())
}

func (actionListWorker *ActionListResponseWorker) UpdateProgress(actionOid int64, actionState proto.ActionResponseState,
	...
	clientMsg := NewClientMessageBuilder(actionListWorker.msgId).SetActionProgress(actionProgress).Create()
	// Send the response on the callback channel to send to the server
	actionListWorker.probeMsgChan <- clientMsg // This will block till the channel is ready to receive
	glog.V(3).Infof("Sent action [%d] progress for  %d.", actionOid, clientMsg.GetMessageID())
}
