...
const (
	...
	writeWaitTimeout                 = 8 * time.Minute
...
type ClientWebSocketTransport struct {
	...
	reconnectNotificationCh  chan error // channel to trigger reconnect of weksocket connection
	service                  string
}
...
func (wsTransport *ClientWebSocketTransport) Connect(refreshTokenChannel chan struct{}, jwTokenChannel chan string) error {
	...
	glog.V(4).Info("[Connect] Connected to server " + wsTransport.GetConnectionId())
	...
	wsTransport.reconnectNotificationCh = make(chan error, 1)
...
func (wsTransport *ClientWebSocketTransport) NotifyReconnect() chan error {
	return wsTransport.reconnectNotificationCh
}
...
func (wsTransport *ClientWebSocketTransport) write(mtype int, payload []byte) error {
	...
	err := ws.WriteMessage(mtype, payload)

	if err != nil && strings.Contains(err.Error(), "i/o timeout") {
		glog.Warningf("websocket write timeout: %v", err)
		wsTransport.reconnectNotificationCh <- err
	}
	return err
}
