...
func LoadProbe(probe *probe.TurboProbe) error {
	...
	glog.Info("Registered " + config.ProbeCategory + "::" + config.ProbeType)
	return nil
}
...
func (mediationContainer *MediationContainer) UpdateDiscoveryConfiguration(chunkSendDelayMillis int, numObjectsPerChunk int) {
	if discoveryResponseHandler, exists := mediationContainer.theRemoteMediationClient.MessageHandlers[DISCOVERY_REQUEST]; exists {
		if handler, ok := any(discoveryResponseHandler).(DiscoveryRequestConfigUpdater); ok {
			handler.UpdateDiscoveryConfiguration(chunkSendDelayMillis, numObjectsPerChunk)
		}
	}
}
