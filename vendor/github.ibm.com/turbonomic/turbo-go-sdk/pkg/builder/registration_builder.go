...
func (builder *ActionMergePolicyBuilder) ForResizeAction(entityType proto.EntityDTO_EntityType,
	resizeMergePolicy *MergePolicyBuilder) *ActionMergePolicyBuilder {
	...
	resizePolicy, err := resizeMergePolicy.BuildResizeMergePolicy()
	if err != nil {
		glog.Errorf("%v", err)
	}
...
func (builder *ActionMergePolicyBuilder) ForHorizontalScaleAction(entityType proto.EntityDTO_EntityType,
	horizontalScaleMergePolicy *MergePolicyBuilder) *ActionMergePolicyBuilder {
	_, exists := builder.ActionMergePolicyMap[entityType]
	if !exists {
	...
	horizontalScalePolicy, err := horizontalScaleMergePolicy.BuildHorizontalScaleMergePolicy()
	if err != nil {
		glog.Errorf("%v", err)
	}
...
type ActionPolicyBuilder struct {
	ActionPolicyMap map[proto.EntityDTO_EntityType]map[proto.ActionItemDTO_ActionType]*proto.ActionPolicyDTO_ActionPolicyElement
}

func NewActionPolicyBuilder() *ActionPolicyBuilder {
	return &ActionPolicyBuilder{
		ActionPolicyMap: make(map[proto.EntityDTO_EntityType]map[proto.ActionItemDTO_ActionType]*proto.ActionPolicyDTO_ActionPolicyElement),
	}
}

// Create an action policy element specifying the execution mode for an Entity type action
func (builder *ActionPolicyBuilder) WithEntityActions(entityType proto.EntityDTO_EntityType,
	...
	if !exists {
		builder.ActionPolicyMap[entityType] =
			make(map[proto.ActionItemDTO_ActionType]*proto.ActionPolicyDTO_ActionPolicyElement)
	}
	entityPolicies, _ := builder.ActionPolicyMap[entityType]
	entityPolicies[actionType] = &proto.ActionPolicyDTO_ActionPolicyElement{
...
// Create an action policy element specifying the execution mode and support for batching for an Entity type action
func (builder *ActionPolicyBuilder) WithEntityActionsThatSupportBatching(entityType proto.EntityDTO_EntityType,
	actionType proto.ActionItemDTO_ActionType,
	actionCapability proto.ActionPolicyDTO_ActionCapability) *ActionPolicyBuilder {
...
func (builder *ActionPolicyBuilder) WithActionPolicyElement(entityType proto.EntityDTO_EntityType,
	actionType proto.ActionItemDTO_ActionType,
	policyElement proto.ActionPolicyDTO_ActionPolicyElement,
) *ActionPolicyBuilder {
	_, exists := builder.ActionPolicyMap[entityType]
	if !exists {
		builder.ActionPolicyMap[entityType] =
	...
	entityPolicies, _ := builder.ActionPolicyMap[entityType]
	entityPolicies[actionType] = &policyElement
