...
const (
	...
	OAuth2TokenScope string = "role:PROBE_ADMIN"
)
...
func (c *APIClient) GetJwtToken(oauth2Token string) (string, error) {
	if oauth2Token == "" {
		glog.V(4).Infof("The oauth2 token is empty")
		return "", nil
	}
	// the format of getting jwtToken with auth requires the following header
	// key: x-oauth2, value: "oauth2"
	// key: x-auth-token, value: oauth2 access token
	request := c.Post().Resource(api.Resource_Type_auth_token).Header("x-oauth2", "oauth2").Header("x-auth-token", oauth2Token)
	// Execute the request
...
func (c *APIClient) GetOAuth2AccessToken() (string, error) {
	...
	writer.WriteField("scope", OAuth2TokenScope)
	err := writer.Close()
	if err != nil {
		return "", fmt.Errorf("failed to Close the writer: %v", err)
	}
	// Create the rest api request
	// the format of get oauth2 access token requires the following in the body, as form-data format
	// key: client_id, value: client id value
	// key: client_secret, value: client secret value
	// key: grant_type, value: client_credentials
	request := c.Post().Resource(api.Resource_Type_oauth2_token).Header("Content-Type", writer.FormDataContentType()).BufferData(payload)
	// Execute the request
	response, err := request.Do()
	if err != nil {
		return "", fmt.Errorf("failed to get oauth2 access token: %s", err)
	}
	if response.statusCode == 401 {
		// When we receive the 401 status code, means that the credentials are not valid.
		// We return error, so getJwtToken() method in tap_service will continue
		// to retry authentication until the credentials are corrected
		return "", fmt.Errorf("OAuth2 service authentication failed using the given client_id and secret. " +
			"Redeploy the secret containing the correct credentials and restart the probe pod")
	}
	if response.statusCode == 502 {
		// When we receive the 502 status code, meaning the oauth2 service is currently not available.
		// We return error, so getJwtToken() method in tap_service will continue
		// to retry authentication until the service is restored
		return "", fmt.Errorf("OAuth2 service is not available [%v:%s]", response.statusCode, response.status)
	}
	if response.statusCode == 403 {
		// When we receive the 403 status code, means that the oauth2 service is currently not available,
		...
		// If the oauth2 service is temporarily not accessible, we have retry in performWebSocketConnection in turbo-go-sdk
		glog.Errorf("OAuth2 service is not accessible or disabled [%v:%s]", response.statusCode, response.status)
		return "", nil
	}
	var oauth2Token OAuth2TokenBody
	err = json.Unmarshal([]byte(response.body), &oauth2Token)
	if err != nil {
		return "", fmt.Errorf("failed to unmarshall get oauth2 token response: %v", err)
	}
