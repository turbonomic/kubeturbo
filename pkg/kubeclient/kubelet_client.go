...
func (client *KubeletClient) ExecuteRequest(ip, nodeName, path string) ([]byte, error) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.KwokClusterTest) {
		return nil, fmt.Errorf("Metrics endpoint not enabled for kwok test")
	}
	...
	if client.forceProxyEndpoint {
		...
		if err != nil && client.kubeClient != nil {
			glog.V(2).Infof("Could not query kubelet endpoint %s on node %s/%s: %v. "+
...
func (client *KubeletClient) callKubeletEndpoint(ip, path string) ([]byte, error) {
	...
	if err != nil {
		glog.V(3).Infof("Could not query kubelet via httpClient for the node %v on the path %v as the error:%v", ip, path, err)
...
func (client *KubeletClient) GetNodeCpuFrequency(node *v1.Node) (float64, error) {
	...
	if mInfo, err := client.GetMachineInfo(ip, node.Name); err == nil {
		...
		glog.Warningf("Could not get CPU frequency for node %s from kubelet: %v. Will try job based getter.",
			node.Name, err)
