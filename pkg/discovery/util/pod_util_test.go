...
var podPhases []v1.PodPhase = []v1.PodPhase{v1.PodPending, v1.PodRunning, v1.PodSucceeded, v1.PodFailed, v1.PodUnknown}

type podPhaseTestMethod func(pod *v1.Pod) bool

func testPodPhase(t *testing.T, testMethod podPhaseTestMethod, testPhase v1.PodPhase) {
	for _, phase := range podPhases {
		t.Run(string(phase), func(t *testing.T) {
			pod := &v1.Pod{Status: v1.PodStatus{Phase: phase}}
			expectedResult := testPhase == phase

			actualResult := testMethod(pod)
...
func TestPodIsSucceeded(t *testing.T) {
	testPodPhase(t, PodIsSucceeded, v1.PodSucceeded)
}

func TestPodIsFailed(t *testing.T) {
	testPodPhase(t, PodIsFailed, v1.PodFailed)
}
