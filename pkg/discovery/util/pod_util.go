...
// Returns a boolean flag indicating whether or not the given pod is controlled by a Job.
// Pods controlled by Jobs are not suspendable, cloneable, or moveable.
// NOTE: CronJobs will generate Jobs which, in turn, generate the associated Pods. As such, there's no need for an IsCronJob method.
func IsJob(pod *api.Pod) bool {
	isJob := isPodCreatedBy(pod, Kind_Job)
	if isJob {
		glog.V(4).Infof("Pod %s/%s is a Job", pod.Namespace, pod.Name)
	}
	return isJob
}
...
func Controllable(pod *api.Pod, mirrorPodDaemon bool) bool {
	controllable := (!IsMirrorPod(pod) || mirrorPodDaemon) && IsControllableFromAnnotation(pod.GetAnnotations())
	if !controllable {
...
// PodIsTerminating checks if a pod is Terminating
func PodIsTerminating(pod *api.Pod) bool {
	return pod.DeletionTimestamp != nil
}

func PodIsSucceeded(pod *api.Pod) bool {
	return pod.Status.Phase == api.PodSucceeded
}

func PodIsFailed(pod *api.Pod) bool {
	return pod.Status.Phase == api.PodFailed
}
...
func GetPodInPhase(podClient v1.PodInterface, name string, phase api.PodPhase) (*api.Pod, error) {
	...
	if pod == nil {
		err = fmt.Errorf("Pod %s not found. The pod may have been deleted since last discovery", name)
		glog.Error(err.Error())
		return nil, err
	}

	if pod.Status.Phase != phase {
		return nil, fmt.Errorf("Pod %s is not in phase %v. The pod phase may have changed since last discovery", name, phase)
	}
...
func GetPodInPhaseByUid(podClient v1.PodInterface, uid string, phase api.PodPhase) (*api.Pod, error) {
	...
	err = fmt.Errorf("Pod with uid %s in phase %v not found.  The pod phase may have changed since last discovery", uid, phase)
...
func GetPodParentInfo(pod *api.Pod) (OwnerInfo, error) {
	...
	if pod.Annotations != nil && len(pod.Annotations) > 0 {
		...
		if value, ok := pod.Annotations[key]; ok {
			...
			return OwnerInfo{
				APIVersion: ref.Reference.APIVersion,
				Kind:       ref.Reference.Kind,
				Namespace:  ref.Reference.Namespace,
...
// GetControllerUID gets controller UID from the given pod and metrics sink.
...
// GetControllerName get controller name from the given pod and metrics sink.
func GetControllerName(pod *api.Pod, metricsSink *metrics.EntityMetricSink) (string, error) {
	podKey := PodKeyFunc(pod)
	ownerNameMetricId := metrics.GenerateEntityStateMetricUID(metrics.PodType, podKey, metrics.Owner)
	ownerNameMetric, err := metricsSink.GetMetric(ownerNameMetricId)
	if err != nil {
		return "", err
	}
	ownerName := ownerNameMetric.GetValue()
	controllerName, ok := ownerName.(string)
	if !ok {
		return "", fmt.Errorf("owner name %v is not a string", ownerName)
	}
...
func GetContainerNames(parent *unstructured.Unstructured) (sets.String, error) {
	...
	names := []string{}
	if !found {
		// some controllers (e.g., VirtualMachineInstance) do not follow the spec.template.spec convention and does not
		// have the list of containers in the spec.
