...
var (
	...
	pod4OwnerMetric    = metrics.NewEntityStateMetric(metrics.PodType, discoveryutil.PodKeyFunc(testPod4), metrics.Owner, controllerName)
	...
	pod5OwnerMetric    = metrics.NewEntityStateMetric(metrics.PodType, discoveryutil.PodKeyFunc(testPod5), metrics.Owner, controllerName)
	...
	pod6OwnerMetric    = metrics.NewEntityStateMetric(metrics.PodType, discoveryutil.PodKeyFunc(testPod6), metrics.Owner, controllerName)
...
func TestContainerSpecMetricsCollector_CollectContainerSpecMetrics_WithoutRequestMetrics(t *testing.T) {
	...
	// Add pod owner, owner UID and node CPU frequency metrics
	metricsSink.AddNewMetricEntries(pod4OwnerMetric, pod4OwnerUIDMetric)
...
func TestContainerSpecMetricsCollector_CollectContainerSpecMetrics_WithRequestMetrics(t *testing.T) {
	...
	metricsSink := metrics.NewEntityMetricSink().WithMaxMetricPointsSize(10)
	// Add pod owner, owner UID and node CPU frequency metrics
	metricsSink.AddNewMetricEntries(pod5OwnerMetric, pod5OwnerUIDMetric, pod6OwnerMetric, pod6OwnerUIDMetric)
