...
func (worker *k8sContainerSpecDiscoveryWorker) WithORMHandler(ormHandler resourcemapping.ORMHandler) *k8sContainerSpecDiscoveryWorker {
...
func (worker *k8sContainerSpecDiscoveryWorker) Do(clusterSummary *repository.ClusterSummary, containerSpecMetrics []*repository.ContainerSpecMetrics,
	utilizationDataAggStrategy, usageDataAggStrategy string) ([]*proto.EntityDTO, []string, []string, []string) {
	...
	containerSpecEntityDTOBuilder := dtofactory.NewContainerSpecDTOBuilder(clusterSummary, containerSpecMetricsMap, utilizationDataAggregator,
		usageDataAggregator).WithORMHandler(worker.ormHandler)
	return containerSpecEntityDTOBuilder.BuildDTOs()
}
