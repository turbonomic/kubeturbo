...
var (
	...
	ns4      = "ns4" // no pods
	ns5      = "ns5" // pods with cumulative cpu data
	...
	kubens5 = repository.CreateDefaultKubeNamespace(cluster1, ns5, "namespace-uuid5")
	...
	// Pod in ns5 on n1
	metric_cpuUsed_pod_n1_ns5 = metrics.NewEntityResourceMetric(etype, util.PodKeyFunc(pod_ns5_n1), metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 5, Timestamp: 1}, {Value: 8, Timestamp: 2}})
	metric_cpuCap_pod_n1_ns5         = metrics.NewEntityResourceMetric(etype, util.PodKeyFunc(pod_ns5_n1), metrics.CPU, metrics.Capacity, nodeCpuCap)
	metric_cpuRequestUsed_pod_n1_ns5 = metrics.NewEntityResourceMetric(etype, util.PodKeyFunc(pod_ns5_n1), metrics.CPURequest, metrics.Used,
		[]metrics.Cumulative{{Value: 5, Timestamp: 1}, {Value: 8, Timestamp: 2}})
	metric_cpuRequestCap_pod_n1_ns5 = metrics.NewEntityResourceMetric(etype, util.PodKeyFunc(pod_ns5_n1), metrics.CPURequest, metrics.Capacity, nodeCpuCap)
	...
	nodeToPodsMap = map[string][]*v1.Pod{
		node1: {pod_ns1_n1, pod_ns2_n1, pod1_ns3_n1, pod2_ns3_n1, pod_ns5_n1},
...
func TestPodMetricsCollectionsWithCumulativeCPU(t *testing.T) {
	metricsSink.AddNewMetricEntries(
		metric_cpuUsed_pod_n1_ns5,
		metric_cpuCap_pod_n1_ns5,
		metric_cpuRequestCap_pod_n1_ns5,
		metric_cpuRequestCap_pod_n1_ns5)

	_ = kubens5.SetResourceCapacity(metrics.CPULimitQuota, 3.0)
	_ = kubens5.SetResourceCapacity(metrics.CPURequestQuota, 2.0)

	clusterSummary := repository.CreateClusterSummary(kubeCluster)
	collector := &MetricsCollector{
		Cluster:     clusterSummary,
		MetricsSink: metricsSink,
		PodList:     []*v1.Pod{pod_ns5_n1},
		Node:        n1,
	}
	podCollection := collector.CollectPodMetrics()
	...
	for _, podsByQuotaMap := range podCollection {
		for _, podMetricsList := range podsByQuotaMap {
			for _, podMetrics := range podMetricsList {
				for rType, points := range podMetrics.Used {
					assert.Equal(t, metrics.CPU, rType)
					for _, point := range points {
						assert.Equal(t, metrics.Point{Value: 0.003, Timestamp: 2}, point)
					}
				}
			}
		}
