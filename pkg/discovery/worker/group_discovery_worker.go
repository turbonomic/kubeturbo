...
func (worker *k8sEntityGroupDiscoveryWorker) Do(entityGroupList []*repository.EntityGroup, sidecarContainerSpecs,
	operatorControlledContainerSpecs, podsWithVolumes, notReadyNodes, mirrorPodUids []string) ([]*proto.GroupDTO, error) {
...
func (worker *k8sEntityGroupDiscoveryWorker) buildContainerSpecGroups(specs []string, groupType containerSpecGroupType) []*proto.GroupDTO {
	...
	id := ""
	displayName := ""
	if groupType == typeSidecars {
		id = fmt.Sprintf("Injected Sidecars/All-ContainerSpecs-%s", worker.targetId)
		displayName = "Injected Sidecars/All ContainerSpecs"
	} else if groupType == typeOperatorControlled {
		id = fmt.Sprintf("Operator Controlled/All-ContainerSpecs-%s", worker.targetId)
		displayName = "Operator Controlled/All ContainerSpecs"
	}
