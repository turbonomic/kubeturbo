...
func (worker *k8sDiscoveryWorker) buildEntityDTOs(currTask *task.Task) ([]*proto.EntityDTO,
	...
	nodeDTOs, notReadyNodes := worker.buildNodeDTOs([]*api.Node{currTask.Node()}, currTask.NodesPods(),
		currTask.HostnameSpreadWorkloads(), currTask.OtherSpreadPods(), currTask.PodstoControllers(), currTask.Cluster(), currTask.Node2NodeGroups())
...
func (worker *k8sDiscoveryWorker) buildNodeDTOs(nodes []*api.Node, nodesPods map[string][]string,
	hostnameSpreadWorkloads sets.String, otherSpreadPods sets.String, podsToControllers map[string]string, clusterSummary *repository.ClusterSummary, node2nodegroup map[string]sets.String) ([]*proto.EntityDTO, []string) {
	...
	// Build entity DTOs for nodes
	return dtofactory.NewNodeEntityDTOBuilder(worker.sink, stitchingManager, clusterSummary).
		WithClusterKeyInjected(worker.config.clusterKeyInjected).
		BuildEntityDTOs(nodes, nodesPods, hostnameSpreadWorkloads, otherSpreadPods, podsToControllers, node2nodegroup)
}
