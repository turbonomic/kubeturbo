...
func (worker *k8sDiscoveryWorker) executeTask(currTask *task.Task) *task.TaskResult {
	...
	entityDTOs, podEntities, sidecarContainerSpecs, podsWithVolumes, notReadyNodes, mirrorPodUids, kubeVirtPodUids, kubeVirtContainerSpecUids := worker.buildEntityDTOs(currTask)
...
func (worker *k8sDiscoveryWorker) buildEntityDTOs(currTask *task.Task) ([]*proto.EntityDTO,
	[]*repository.KubePod, []string, []string, []string, []string, []string, []string) {
	...
	if len(podDTOs) == 0 {
		return entityDTOs, nil, nil, nil, nil, nil, nil, nil
	}
	...
	return entityDTOs, podEntities, sidecarContainerSpecs, podWithVolumes, notReadyNodes, mirrorPodUids, kubeVirtPodUids, kubeVirtContainerSpecUids
}
