...
func (worker *k8sDiscoveryWorker) WithORMHandler(ormHandler *resourcemapping.ORMHandler) *k8sDiscoveryWorker {
	worker.ormHandler = ormHandler
	return worker
}
...
func (worker *k8sDiscoveryWorker) executeTask(currTask *task.Task) *task.TaskResult {
	...
	entityDTOs, podEntities, sidecarContainerSpecs, podsWithVolumes, notReadyNodes, mirrorPodUids, kubeVirtPodUids,
		kubeVirtContainerSpecUids, unmovablePods := worker.buildEntityDTOs(currTask)
	...
	return task.NewTaskResult(worker.id, task.TaskSucceeded).
		...
		// list of KubeVirt pod UIDs
		WithKubeVirtPodUids(kubeVirtPodUids).
		// list of KubeVirt pod UIDs
		WithKubeVirtContainerSpecUids(kubeVirtContainerSpecUids).
		// list of unmovable pods
		WithUnmovablePods(unmovablePods)
}
...
func (worker *k8sDiscoveryWorker) buildEntityDTOs(currTask *task.Task) ([]*proto.EntityDTO,
	[]*repository.KubePod, []string, []string, []string, []string, []string, []string, []string) {
	...
	podDTOs, runningPods, podWithVolumes, mirrorPodUids, podsWithUnsupportedControllers := worker.buildPodDTOs(currTask)
	glog.V(3).Infof("Worker %s built %d pod DTOs.", worker.id, len(podDTOs))
	if len(podDTOs) == 0 {
		return entityDTOs, nil, nil, nil, nil, nil, nil, nil, nil
	}
	entityDTOs = append(entityDTOs, podDTOs...)
	// Build entity DTOs for containers from running pods
	containerDTOs, sidecarContainerSpecs, kubeVirtPodUids, kubeVirtContainerSpecUids := worker.buildContainerDTOs(currTask.Cluster(), runningPods)
	...
	return entityDTOs, podEntities, sidecarContainerSpecs, podWithVolumes, notReadyNodes, mirrorPodUids,
		kubeVirtPodUids, kubeVirtContainerSpecUids, podsWithUnsupportedControllers
}
...
func (worker *k8sDiscoveryWorker) buildNodeDTOs(nodes []*api.Node, nodesPods map[string][]string,
	...
	clusterSummary *repository.ClusterSummary, node2nodegroup map[string]sets.String, affinityMapper *podaffinity.AffinityMapper,
) ([]*proto.EntityDTO, []string) {
...
func (worker *k8sDiscoveryWorker) buildPodDTOs(currTask *task.Task) ([]*proto.EntityDTO, []*api.Pod, []string, []string, []string) {
	...
	runningPodDTOs, pendingPodDTOs, podsWithVolumes, mirrorPodUids, podsWithUnsupportedControllers := dtofactory.
		...
		WithORMHandler(*currTask.ORMHandler()).
		BuildEntityDTOs()
	...
	return podDTOs, runningPods, podsWithVolumes, mirrorPodUids, podsWithUnsupportedControllers
}

// Build DTOs for containers
func (worker *k8sDiscoveryWorker) buildContainerDTOs(cluster *repository.ClusterSummary, runningPods []*api.Pod) ([]*proto.EntityDTO, []string, []string, []string) {
	return dtofactory.
