...
func (pi *PodInfo) DeepCopy() *PodInfo {
	return &PodInfo{
		...
		TopologySpreadConstraints: pi.TopologySpreadConstraints,
	}
}
...
func (pi *PodInfo) Update(pod *v1.Pod) error {
	...
	podTopologySpreadConstraints := getRequiredPodTopologySpreadConstraints(pod.Spec.TopologySpreadConstraints)
	...
	pi.TopologySpreadConstraints = podTopologySpreadConstraints
...
func getRequiredPodTopologySpreadConstraints(topologySpreadConstraints []v1.TopologySpreadConstraint) []v1.TopologySpreadConstraint {
	if len(topologySpreadConstraints) > 0 {
		requiredTopologySpreadConstraints := []v1.TopologySpreadConstraint{}
		for _, constraint := range topologySpreadConstraints {
			if constraint.WhenUnsatisfiable == v1.DoNotSchedule {
				requiredTopologySpreadConstraints = append(requiredTopologySpreadConstraints, constraint)
			}
		}
		return requiredTopologySpreadConstraints
...
func podWithRequiredPodAffinity(p *v1.Pod) bool {
	affinity := p.Spec.Affinity
	return affinity != nil && affinity.PodAffinity != nil &&
