...
func TestRequiredAffinityMultipleNodes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "A pod cannot be scheduled on any node if it has affinity rule and there are no pods that match the rule - no pods",
			...
			expectNotPlacedOn: sets.NewString("node1", "node2", "node3"),
		},
		{
			name:       "A pod cannot be scheduled on any node if it has affinity rule and there are no pods that match the rule - a pod exists but doesn't match selector",
			podToPlace: at.MakePod().Name("podToPlace").Namespace(defaultNamespace).PodAffinityIn("foo", "region", []string{"bar"}, at.PodAffinityWithRequiredReq).Obj(),
			pods: []*v1.Pod{
				at.MakePod().Name("p1").Node("node1").Labels(map[string]string{
					"foo": "NOT-bar",
				}).Obj(),
			},
			nodes: []*v1.Node{
