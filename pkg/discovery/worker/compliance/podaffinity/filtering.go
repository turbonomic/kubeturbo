...
func (pr *PodAffinityProcessor) getExistingAntiAffinityCounts(ctx context.Context, pod *v1.Pod,
	...
	for _, nodeInfo := range nodes {
		...
		for _, existingPod := range nodeInfo.PodsWithRequiredAntiAffinity {
			...
			_, hostnameSpread, termsToConsiderFurther, _ := pr.podAntiAffinityMatchesItsReplica(existingPod, pod, nsLabels, node)
			if !hostnameSpread {
...
func (pr *PodAffinityProcessor) podAntiAffinityMatchesItsReplica(existingPod *PodInfo, thisPod *v1.Pod,
	nsLabels labels.Set, node *v1.Node) (bool, bool, []AffinityTerm, []AffinityTerm) {
	...
	return true, hostnameSpread, nonMatchingTerms, nonSpreadTerms
}
...
func (pr *PodAffinityProcessor) getIncomingAffinityAntiAffinityCounts(ctx context.Context, podInfo *PodInfo, allNodes []*NodeInfo) (topologyToMatchedTermCount, topologyToMatchedTermCount) {
	...
	for _, nodeInfo := range allNodes {
		...
		for _, existingPod := range nodeInfo.Pods {
			...
			_, hostnameSpread, termsToConsiderFurther, _ := pr.podAntiAffinityMatchesItsReplica(podInfo, existingPod.Pod, nil, node)
...
func (pr *PodAffinityProcessor) ProcessAffinityMappings(pod *v1.Pod) error {
	...
	pr.processThisPodWrtAllOthersOnNode(podInfo, allNodes)
...
func (pr *PodAffinityProcessor) processThisPodWrtAllOthersOnNode(podInfo *PodInfo, allNodes []*NodeInfo) {
	for _, nodeInfo := range allNodes {
		node := nodeInfo.Node()
		if node == nil {
			klog.ErrorS(nil, "Node not found")
		...
		for _, podOnThisNode := range nodeInfo.Pods {
			...
			sameParent, _, _, nonSpreadTerms := pr.podAntiAffinityMatchesItsReplica(podInfo, podOnThisNode.Pod, nil, node)
			if !sameParent {
				nonSpreadTerms = podInfo.RequiredAntiAffinityTerms
			}
			pr.affinityMapper.BuildAffinityMaps(podInfo.RequiredAffinityTerms, podInfo, podOnThisNode, node, Affinity)
