...
// AffinityWrapper wraps a Affinity inside.
//
// TODO: I'm adding our own wrapper enrichment functions here, as we have modified from the exact copy we borrow from
// upstream.  I wonder how the long-term maintenance strategy; I personally think our additions should be separated
// from the original copy which can then be easily maintained by simply updating to a new copy as needed when changes
// are made in upstream.
type AffinityWrapper struct{ v1.Affinity }

// MakeAffinity creates an Affinity wrapper.
func MakeAffinity() *AffinityWrapper {
	return &AffinityWrapper{v1.Affinity{}}
}

// MakeAffinityWrapper creates an Affinity wrapper from an existing Affinity.  It creates a new one if the existing one
// is nil.
func MakeAffinityWrapper(affinity *v1.Affinity) *AffinityWrapper {
	if affinity == nil {
		return MakeAffinity()
	}
	return &AffinityWrapper{*affinity}
}

// AddSingleNodeAffinityExpression appends a single node affinity expression in the required section.
func (a *AffinityWrapper) AddSingleNodeAffinityExpression(nodeName string) *AffinityWrapper {
	expression := v1.NodeSelectorRequirement{
		Key:      "kubernetes.io/hostname",
		Operator: v1.NodeSelectorOpIn,
		Values:   []string{nodeName},
	}
	return a.AddNodeAffinityExpressionToAllRequired(expression)
}

// RemoveSingleNodeAffinityExpression removes a single node affinity expression from the required section.
func (a *AffinityWrapper) RemoveSingleNodeAffinityExpression(nodeName string) *AffinityWrapper {
	expression := v1.NodeSelectorRequirement{
		Key:      "kubernetes.io/hostname",
		Operator: v1.NodeSelectorOpIn,
		Values:   []string{nodeName},
	}
	return a.RemoveNodeAffinityExpressionFromAllRequired(expression)
}

// AddNodeAffinityExpressionToAllRequired adds the expression in the required node affinity section to all ORed node
// selector terms.
func (a *AffinityWrapper) AddNodeAffinityExpressionToAllRequired(expression v1.NodeSelectorRequirement) *AffinityWrapper {
	a.NodeAffinity = MakeNodeAffinityWrapper(a.NodeAffinity).AddExpressionToAllRequired(expression).Obj()
	return a
}

// RemoveNodeAffinityExpressionFromAllRequired removes the expression from all ORed node selector terms in the required
// node affinity section.
func (a *AffinityWrapper) RemoveNodeAffinityExpressionFromAllRequired(expression v1.NodeSelectorRequirement) *AffinityWrapper {
	a.NodeAffinity = MakeNodeAffinityWrapper(a.NodeAffinity).RemoveExpressionFromAllRequired(expression).Obj()
...
// Obj returns the inner Affinity.
func (a *AffinityWrapper) Obj() *v1.Affinity {
	if a == nil {
		return nil
	}
	return &a.Affinity
...
// NodeAffinityWrapper wraps a NodeAffinity inside.
type NodeAffinityWrapper struct{ v1.NodeAffinity }

// MakeNodeAffinity creates a NodeAffinity wrapper.
func MakeNodeAffinity() *NodeAffinityWrapper {
	return &NodeAffinityWrapper{v1.NodeAffinity{}}
}

// MakeNodeAffinityWrapper creates a NodeAffinity wrapper from an existing NodeAffinity.  It creates a new one if the
// existing one is nil.
func MakeNodeAffinityWrapper(nodeAffinity *v1.NodeAffinity) *NodeAffinityWrapper {
	if nodeAffinity == nil {
		return MakeNodeAffinity()
	}
	return &NodeAffinityWrapper{*nodeAffinity}
}

// AddExpressionToAllRequired appends the expression in the required section to all ORed node selector terms.
func (na *NodeAffinityWrapper) AddExpressionToAllRequired(expression v1.NodeSelectorRequirement) *NodeAffinityWrapper {
	na.RequiredDuringSchedulingIgnoredDuringExecution = MakeNodeSelectorWrapper(
		na.RequiredDuringSchedulingIgnoredDuringExecution).AddExpressionToAll(expression).Obj()
	return na
}

// RemoveExpressionFromAllRequired removes the expression from all ORed node selector terms in the required section.
func (na *NodeAffinityWrapper) RemoveExpressionFromAllRequired(expression v1.NodeSelectorRequirement) *NodeAffinityWrapper {
	na.RequiredDuringSchedulingIgnoredDuringExecution = MakeNodeSelectorWrapper(
		na.RequiredDuringSchedulingIgnoredDuringExecution).RemoveExpressionFromAll(expression).Obj()
	if na.RequiredDuringSchedulingIgnoredDuringExecution == nil && na.PreferredDuringSchedulingIgnoredDuringExecution == nil {
		return nil
	}
	return na
}

// Obj returns the inner NodeAffinity.
func (na *NodeAffinityWrapper) Obj() *v1.NodeAffinity {
	if na == nil {
		return nil
	}
	return &na.NodeAffinity
}
...
// MakeNodeSelectorWrapper creates a NodeSelector wrapper from an existing NodeSelector.  It creates a new one if the
// existing one is nil.
func MakeNodeSelectorWrapper(nodeSelector *v1.NodeSelector) *NodeSelectorWrapper {
	if nodeSelector == nil {
		return MakeNodeSelector()
	}
	return &NodeSelectorWrapper{*nodeSelector}
}

// AddExpressionToAll appends the expression to all ORed selector terms and returns the updated wrapper.
// Node selector specification is a list of lists.  Each inner list is a list of ANDed conditions, while the outer list
// is an ORed of all inner lists.  Therefore, when we append an expression, we will append it to each the inner list.
// If there are no selector terms, one will be created with the given expression.
func (s *NodeSelectorWrapper) AddExpressionToAll(expression v1.NodeSelectorRequirement) *NodeSelectorWrapper {
	if s.NodeSelectorTerms == nil {
		s.NodeSelectorTerms = []v1.NodeSelectorTerm{}
	}
	if len(s.NodeSelectorTerms) == 0 {
		s.NodeSelectorTerms = append(s.NodeSelectorTerms, v1.NodeSelectorTerm{})
	}
	for i := range s.NodeSelectorTerms {
		s.NodeSelectorTerms[i].MatchExpressions = append(s.NodeSelectorTerms[i].MatchExpressions, expression)
		i = i + 1
	}
	return s
}

// RemoveExpressionFromAll removes the expression if found from all ORed selector terms and returns the updated wrapper.
// Node selector specification is a list of lists.  Each inner list is a list of ANDed conditions, while the outer list
// is an ORed of all inner lists.  Therefore, when we remove an expression, we will remove it from each the inner list.
// This will only remove the first matched one from each list.
func (s *NodeSelectorWrapper) RemoveExpressionFromAll(expression v1.NodeSelectorRequirement) *NodeSelectorWrapper {
	var newNodeSelectorTerms []v1.NodeSelectorTerm
	for i := range s.NodeSelectorTerms {
		matchExpressions := s.NodeSelectorTerms[i].MatchExpressions
		removeIndex := -1
		for j, matchExpression := range matchExpressions {
			if reflect.DeepEqual(matchExpression, expression) {
				removeIndex = j
				break
			}
		}
		if removeIndex >= 0 {
			newMatchExpressions := append(matchExpressions[:removeIndex], matchExpressions[removeIndex+1:]...)
			s.NodeSelectorTerms[i].MatchExpressions = newMatchExpressions
		}
		if len(s.NodeSelectorTerms[i].MatchExpressions) > 0 || len(s.NodeSelectorTerms[i].MatchFields) > 0 {
			newNodeSelectorTerms = append(newNodeSelectorTerms, s.NodeSelectorTerms[i])
		}
	}
	s.NodeSelectorTerms = newNodeSelectorTerms
	if len(s.NodeSelectorTerms) == 0 {
		return nil
	}
	return s
...
func (p *PodWrapper) Res(resMap map[v1.ResourceName]string) *PodWrapper {
	...
	name := fmt.Sprintf("con%d", len(p.Spec.Containers))
	p.Spec.Containers = append(p.Spec.Containers, MakeContainer().Name(name).Image(BusyboxImage).Resources(resMap).Obj())
	return p
}
...
func (p *PodWrapper) Req(reqMap map[v1.ResourceName]string) *PodWrapper {
	...
	name := fmt.Sprintf("con%d", len(p.Spec.Containers))
	p.Spec.Containers = append(p.Spec.Containers, MakeContainer().Name(name).Image(BusyboxImage).ResourceRequests(reqMap).Obj())
	return p
}
...
func (p *PodWrapper) Lim(limMap map[v1.ResourceName]string) *PodWrapper {
	...
	name := fmt.Sprintf("con%d", len(p.Spec.Containers))
	p.Spec.Containers = append(p.Spec.Containers, MakeContainer().Name(name).Image(BusyboxImage).ResourceLimits(limMap).Obj())
	return p
}
...
func (p *PodWrapper) InitReq(resMap map[v1.ResourceName]string) *PodWrapper {
	...
	p.Spec.InitContainers = append(p.Spec.InitContainers, MakeContainer().Name(name).Image(BusyboxImage).Resources(resMap).Obj())
	return p
}
