...
type NodeInfoLister interface {
	...
	List() ([]*NodeInfo, error)
	// Get returs nodeInfo for a given node
	Get(name string) (*NodeInfo, error)
...
func NewNodeInfoLister(clusterSummary *repository.ClusterSummary) NodeInfoLister {
	...
	for _, node := range clusterSummary.Nodes {
		...
		lister.nodeInfoMap[node.Name] = nodeInfo
...
func (n *NodeInfoListerImpl) Get(name string) (*NodeInfo, error) {
	node, found := n.nodeInfoMap[name]
	if found {
		return node, nil
	}
	return nil, fmt.Errorf("node %s not found in lister", name)
}
...
func NewNamespaceLister(client *client.Clientset, clusterSummary *repository.ClusterSummary, stopCh <-chan struct{}) (listersv1.NamespaceLister, error) {
