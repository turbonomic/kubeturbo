...
func TestBuildDTOsWithMissingMetrics(t *testing.T) {
	...
	clusterId := "foo"
	cluster := repository.NewKubeCluster(clusterId, []*api.Node{node})
	clusterSummary := repository.CreateClusterSummary(cluster)

	currTask := task.NewTask().WithCluster(clusterSummary).WithNode(node).WithPods([]*api.Pod{pod})

	entityDtos, kubePods, _, _, _, _, _, _, _ := worker.buildEntityDTOs(currTask)
