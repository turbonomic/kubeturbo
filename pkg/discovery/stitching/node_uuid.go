...
func (d *defaultNodeUUIDGetter) GetUUID(node *api.Node) (string, error) {
	...
	if err != nil {
		glog.Warningf("Could not reverse endianness of node %s's UUID %s: %v", node.Name, suuid, err)
		return suuid, nil
	} else {
...
func (azure *azureNodeUUIDGetter) GetUUID(node *api.Node) (string, error) {
	...
	if !strings.HasPrefix(providerId, azurePrefix) && satelliteProvider != "azure" {
	...
	reversedSuuid, err := reverseUuid(suuid)
	if err != nil {
		glog.Warningf("Could not reverse endianness of Azure node %s's UUID %s: %v", node.Name, suuid, err)
		return result, nil
	} else {
...
func (v *vsphereNodeUUIDGetter) GetUUID(node *api.Node) (string, error) {
	...
	if err != nil {
		glog.Warningf("Could not reverse endianness of Vsphere node %s's UUID %s: %v", node.Name, providerID, err)
	} else {
	...
	if suuid != "" && suuid != providerID {
		...
		reversedSuuid, err := reverseUuid(suuid)
		if err != nil {
			glog.Warningf("Could not reverse endianness of Vsphere node %s's UUID %s: %v", node.Name, suuid, err)
		} else {
