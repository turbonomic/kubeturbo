...
func (s *StitchingManager) SetNodeUuidGetterByProvider(providerId string) {
	...
	var satelliteProvider string
	if pflag.Lookup("satellite-location-provider") != nil {
		satelliteProvider = pflag.Lookup("satellite-location-provider").Value.String()
	}
	...
	if strings.HasPrefix(providerId, vspherePrefix) {
		...
		(strings.HasPrefix(providerId, ibmPrefix) && satelliteProvider == "azure") {
	...
	if satelliteProvider != "" {
		glog.V(4).Infof("Satellite locaction provider is %v, Node providerId is %v, Node UUID getter is: %v", satelliteProvider, providerId, getter.Name())
	}
}
...
func (s *StitchingManager) storeNodeUUID(node *api.Node) {
	...
	if err != nil || nodeStitchingID == "" {
		if utilfeature.DefaultFeatureGate.Enabled(features.KwokClusterTest) {
			// we skip stitching KWOK fake node
			return
		} else {
...
func (s *StitchingManager) GenerateReconciliationMetaData() (*proto.EntityDTO_ReplacementEntityMetaData, error) {
	...
	replacementEntityMetaDataBuilder.PatchSellingWithProperty(proto.CommodityDTO_CLUSTER, capacityOnlyPropertyNames).
		...
		PatchSellingWithProperty(proto.CommodityDTO_PEER_TO_PEER_AFFINITY, usedAndCapacityAndPeakPropertyNames).
		PatchSellingWithProperty(proto.CommodityDTO_PEER_TO_PEER_ANTI_AFFINITY, usedAndCapacityAndPeakPropertyNames).
