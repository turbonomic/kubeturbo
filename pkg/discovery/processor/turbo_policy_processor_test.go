...
var (
	...
	queuingTime                   = 300.2
	serviceTime                   = 300.3
	concurrentQueries             = 13
	llmCache                      = 70
	marshaledRT, _                = json.Marshal(responseTime)
	marshaledTRX, _               = json.Marshal(transaction)
	marshaledQueuingTime, _       = json.Marshal(queuingTime)
	marshaledServiceTime, _       = json.Marshal(serviceTime)
	marshaledConcurrentQueries, _ = json.Marshal(concurrentQueries)
	marshaledLlmCache, _          = json.Marshal(llmCache)
	...
	sloScaleSpec = policyv1alpha1.SLOHorizontalScaleSpec{
		...
		Objectives: []policyv1alpha1.SLOHorizontalScalePolicySetting{
			...
			{
				Name:  "QueuingTime",
				Value: apiextensionv1.JSON{Raw: marshaledQueuingTime},
			},
			{
				Name:  "ServiceTime",
				Value: apiextensionv1.JSON{Raw: marshaledServiceTime},
			},
			{
				Name:  "ConcurrentQueries",
				Value: apiextensionv1.JSON{Raw: marshaledConcurrentQueries},
			},
			{
				Name:  "LLMCache",
				Value: apiextensionv1.JSON{Raw: marshaledLlmCache},
			},
		},
...
func TestConvertTurboPolicy(t *testing.T) {
	...
	validSettingType := []proto.GroupDTO_Setting_SettingType{
		...
		proto.GroupDTO_Setting_SERVICE_TIME_SLO,
		proto.GroupDTO_Setting_QUEUING_TIME_SLO,
		proto.GroupDTO_Setting_CONCURRENT_QUERIES_SLO,
		proto.GroupDTO_Setting_LLM_CACHE_SLO,
	...
	for _, groupDTO := range groupDTOs {
		...
		for _, setting := range settings {
			...
			if setting.GetType() == proto.GroupDTO_Setting_QUEUING_TIME_SLO {
				assert.Equal(t, float32(queuingTime), setting.GetNumericSettingValueType().GetValue())
			}
			if setting.GetType() == proto.GroupDTO_Setting_SERVICE_TIME_SLO {
				assert.Equal(t, float32(serviceTime), setting.GetNumericSettingValueType().GetValue())
			}
			if setting.GetType() == proto.GroupDTO_Setting_CONCURRENT_QUERIES_SLO {
				assert.Equal(t, float32(concurrentQueries), setting.GetNumericSettingValueType().GetValue())
			}
			if setting.GetType() == proto.GroupDTO_Setting_LLM_CACHE_SLO {
				assert.Equal(t, float32(llmCache), setting.GetNumericSettingValueType().GetValue())
			}
		}
	}
	kubeCluster.TurboPolicyBindings = nil
