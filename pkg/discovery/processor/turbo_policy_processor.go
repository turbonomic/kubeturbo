...
func (p *TurboPolicyProcessor) ProcessTurboPolicies() {
	...
	if err != nil {
		glog.Warningf("Could to list SLOHorizontalScales: %v.", err)
	}

	turboCvsScalings, err := p.ClusterScraper.GetAllTurboCVSScalings()
	if err != nil {
		glog.Warningf("Could to list ContainerVerticalScales: %v.", err)
	}
	...
	if err != nil {
		glog.Warningf("Could to list PolicyBindings: %v.", err)
		return
	}
