...
func Test_containerSpecDTOBuilder_getCommoditiesSold(t *testing.T) {
	...
	containerSpecDTOs, operatorControlledContainerSpecs, systemNameSpaceContainerSpecs, unresizableContainerSpecs := builder.BuildDTOs()
	assert.NotNil(t, operatorControlledContainerSpecs)
	assert.Nil(t, systemNameSpaceContainerSpecs)
	assert.Nil(t, unresizableContainerSpecs)
	assert.True(t, len(containerSpecDTOs) == 1)

	properties := containerSpecDTOs[0].EntityProperties
	actualNamespace, err := property.GetNamespaceFromProperties(properties)
	assert.Nil(t, err)
	assert.Equal(t, namespace, actualNamespace)

	actualWorkloadControllerName, err := property.GetWorkloadControllerNameFromProperties(properties)
	assert.Nil(t, err)
	assert.Equal(t, controllerName, actualWorkloadControllerName)

	actualContainerName, err := property.GetContainerNameFromProperties(properties)
	assert.Nil(t, err)
	assert.Equal(t, containerSpecName, actualContainerName)

	actualFqn, err := property.GetFullyQualifiedNameFromProperties(properties)
	assert.Nil(t, err)
	assert.Equal(t, strings.Join([]string{clusterName, namespace, controllerName, containerSpecName}, util.NamingQualifierSeparator), actualFqn)
}
