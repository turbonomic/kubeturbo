...
func TestMetricValueWithMultiCumulativePoints(t *testing.T) {
	metricsSink = metrics.NewEntityMetricSink().WithMaxMetricPointsSize(10)
	containerId := "container-cumulative"
	cpuUsedMetric1 := metrics.NewEntityResourceMetric(metrics.ContainerType, containerId, metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 5, Timestamp: 1}})
	cpuUsedMetric2 := metrics.NewEntityResourceMetric(metrics.ContainerType, containerId, metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 8, Timestamp: 2}})
	cpuUsedMetric3 := metrics.NewEntityResourceMetric(metrics.ContainerType, containerId, metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 10, Timestamp: 3}})
	cpuUsedMetric4 := metrics.NewEntityResourceMetric(metrics.ContainerType, containerId, metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 15, Timestamp: 4}})
	cpuUsedMetric5 := metrics.NewEntityResourceMetric(metrics.ContainerType, containerId, metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 25, Timestamp: 5}})
	metricsSink.AddNewMetricEntries(cpuUsedMetric1)
	metricsSink.UpdateMetricEntry(cpuUsedMetric2)
	metricsSink.UpdateMetricEntry(cpuUsedMetric3)
	...
	metricValue, _ := dtoBuilder.metricValue(metrics.ContainerType, containerId, metrics.CPU, metrics.Used, nil)
	// total = (8-5) (10-8) (15-10) (25-15)
	// Avg = (3 + 2 + 5 + 10) / 4e3 = 0.005
	// Peak = 10 / 1e3 = 0.01
	assert.EqualValues(t, 0.005, metricValue.Avg)
	assert.EqualValues(t, 0.01, metricValue.Peak)
}

func TestMetricValueWithSingleCumulativePoints(t *testing.T) {
	metricsSink = metrics.NewEntityMetricSink().WithMaxMetricPointsSize(10)
	containerId := "container-cumulative"
	cpuUsedMetric1 := metrics.NewEntityResourceMetric(metrics.ContainerType, containerId, metrics.CPU, metrics.Used,
		[]metrics.Cumulative{{Value: 5, Timestamp: 1}})
	...
	metricValue, err := dtoBuilder.metricValue(metrics.ContainerType, containerId, metrics.CPU, metrics.Used, nil)
	assert.NotNil(t, err)
	assert.EqualValues(t, 0, int(metricValue.Avg))
	assert.EqualValues(t, 0, int(metricValue.Peak))
}
...
func TestMetricValueWithThrottlingCumulativePoints(t *testing.T) {
	...
	assert.InDelta(t, 33.33, metricValue.Avg, 1e-2)
	assert.InDelta(t, 50, metricValue.Peak, 1e-2)
