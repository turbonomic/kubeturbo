...
func (builder *podEntityDTOBuilder) getPodCommoditiesBoughtFromNode(pod *api.Pod,
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.NewAffinityProcessing) {
		affinityComms, err := builder.getAffinityRelatedCommodities(pod.Namespace+"/"+pod.Name, true)
		if err != nil {
			return nil, err
		}
...
func (builder *podEntityDTOBuilder) getPodCommoditiesBoughtFromNodeGroup(pod *api.Pod) ([]*proto.CommodityDTO, error) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.NewAffinityProcessing) {
		...
		affinityComms, err := builder.getAffinityRelatedCommodities(pod.Namespace+"/"+pod.Name, false)
		if err != nil {
			return nil, err
		}
		commoditiesBought = append(commoditiesBought, affinityComms...)
...
func (builder *podEntityDTOBuilder) getAffinityRelatedCommodities(podQualifiedName string, isFromNode bool) ([]*proto.CommodityDTO, error) {
	...
	if isFromNode {
		...
		// hostnameSpreadPods will be empty if NewAffinityProcessing is not enabled
		...
		if !exists {
			glog.Warningf("Topology key for spread workload pod %s not found", podQualifiedName)
			return affinityComms, nil
		}
...
func (builder *podEntityDTOBuilder) SetCommoditiesBoughtFromNodeGroup(entityDTOBuilder *sdkbuilder.EntityDTOBuilder, commoditiesBoughtNodeGroup []*proto.CommodityDTO, nodeId string) {
	ngId2segcommodities := make(map[string][]*proto.CommodityDTO) // Map of nodeid ----> segmentation commodities
	for _, segcommodity := range commoditiesBoughtNodeGroup {
		keyparts := strings.Split(segcommodity.GetKey(), "@")
		if len(keyparts) > 0 {
			tpkey := keyparts[0]
			for nodeGroupId := range builder.node2nodegroups[nodeId] {
				...
				if len(ngparts) > 0 && ngparts[0] == tpkey {
					...
					commoditiesLst = append(commoditiesLst, segcommodity)
