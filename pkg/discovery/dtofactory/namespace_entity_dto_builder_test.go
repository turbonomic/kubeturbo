...
func TestBuildNamespaceDto(t *testing.T) {
	...
	clusterSummary := mockClusterSummaryWithClusterIdOnly(clusterName)
	clusterSummary.NamespaceMap = namespaceMap
	...
	builder := NewNamespaceEntityDTOBuilder(clusterSummary)
	...
	for _, dto := range dtos {
		...
		namespaceFromProperties, err := property.GetNamespaceFromProperties(dto.EntityProperties)
		assert.Nil(t, err)
		assert.Equal(t, kubeNamespace.Name, namespaceFromProperties)
		fqnFromProperties, err := property.GetFullyQualifiedNameFromProperties(dto.EntityProperties)
		assert.Nil(t, err)
		assert.Equal(t, clusterName+util.NamingQualifierSeparator+kubeNamespace.Name, fqnFromProperties)
	}
...
// Ensure that Namespaces that have a temporarily resized ResourceQuota are marked as non-controllable
// so that quota resize actions aren't generated by the market.
func TestBuildNamespaceEntityDtoWithTemporaryResourceQuota(t *testing.T) {
	namespaceName := "namespace"
	namespaceUID := types.UID(namespaceName)
	configuredCpuLimit := "100m"
	cpuUsage := "200m"

	quotaCapacity := v1.ResourceList{
		v1.ResourceLimitsCPU: k8sres.MustParse(configuredCpuLimit),
	}
	quotaUsage := v1.ResourceList{
		v1.ResourceLimitsCPU: k8sres.MustParse(cpuUsage),
	}
	...
	// resized quota
	encodedQuota, err := util.EncodeQuota(configuredQuota)
	assert.Nil(t, err, "Failed to encode quota")

	// Mock the temporarily resized quota containing the annotation with the original config
	resizedQuota := &v1.ResourceQuota{
		ObjectMeta: metav1.ObjectMeta{
			Name:        namespaceName,
			UID:         namespaceUID,
			Namespace:   namespaceName,
			Annotations: map[string]string{util.QuotaAnnotationKey: encodedQuota},
		},
		Status: v1.ResourceQuotaStatus{
			Hard: quotaCapacity,
			Used: quotaUsage,
	...
	quotas := []*v1.ResourceQuota{resizedQuota}

	namespace := repository.CreateDefaultKubeNamespace("cluster", namespaceName, string(namespaceUID))
	namespace.ReconcileQuotas(quotas)
	namespace.QuotaList = append(namespace.QuotaList, resizedQuota)

	clusterSummary := mockClusterSummaryWithClusterIdOnly(clusterName)
	clusterSummary.NamespaceMap = map[string]*repository.KubeNamespace{namespaceName: namespace}

	builder := NewNamespaceEntityDTOBuilder(clusterSummary)
	dtos, err := builder.BuildEntityDTOs()
	assert.Nil(t, err)

	// Assert that the namespace is NOT controllable (i.e. action generation is disabled)
	for _, dto := range dtos {
		assert.NotNil(t, dto.ConsumerPolicy)
		assert.False(t, *dto.ConsumerPolicy.Controllable)
	}
}
