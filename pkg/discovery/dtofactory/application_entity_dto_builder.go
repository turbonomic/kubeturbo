...
func NewApplicationEntityDTOBuilder(sink *metrics.EntityMetricSink,
	clusterSummary *repository.ClusterSummary, clusterScraper *cluster.ClusterScraper) *applicationEntityDTOBuilder {
	return &applicationEntityDTOBuilder{
		generalBuilder:           newGeneralBuilder(sink, clusterSummary),
		podClusterIDToServiceMap: clusterSummary.PodClusterIDToServiceMap,
...
func (builder *applicationEntityDTOBuilder) BuildEntityDTO(pod *api.Pod) ([]*proto.EntityDTO, error) {
	...
	for i := range pod.Spec.Containers {
		...
		//4. set properties
		properties := builder.getApplicationProperties(pod, container, i, svcUID)
...
func (builder *applicationEntityDTOBuilder) getApplicationProperties(pod *api.Pod, container *api.Container, index int, svcUID string) []*proto.EntityDTO_EntityProperty {
	var properties []*proto.EntityDTO_EntityProperty
	// additional node cluster info property.
	appProperties := property.AddContainerProperties(builder.clusterSummary.Name, pod.Namespace, pod.Name, container.Name, index)
