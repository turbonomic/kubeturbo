...
import (
	"fmt"
	pkgutil "github.ibm.com/turbonomic/kubeturbo/pkg/util"
	"strings"
...
func NewContainerDTOBuilder(sink *metrics.EntityMetricSink, cluster *repository.ClusterSummary) *containerDTOBuilder {
	return &containerDTOBuilder{
		generalBuilder: newGeneralBuilder(sink, cluster),
	}
}

func (builder *containerDTOBuilder) BuildEntityDTOs(pods []*api.Pod) (result []*proto.EntityDTO, sidecars []string, kubeVirtPodUids []string, kubeVirtContainerSpecUids []string) {
	var err error
	...
	for _, pod := range pods {
		...
		isKubevirtPod := false
		if !utilfeature.DefaultFeatureGate.Enabled(features.VirtualMachinePodResize) {
			ownerInfo, err := util.GetPodParentInfo(pod)
			if err == nil && ownerInfo.Kind == pkgutil.KindVirtualMachineInstance {
				isKubevirtPod = true
				kubeVirtPodUids = append(kubeVirtPodUids, podId)
			}
		}
		...
		for i := range pod.Spec.Containers {
			...
			if controllerUID != "" {
				...
				if builder.isInjectedSidecarContainer(containerMId) && containerIsNotKubevirtVmContainer(container) {
					// Add the containerSpec id to the set
					sidecars = append(sidecars, containerSpecId)
				}
				if isKubevirtPod {
					kubeVirtContainerSpecUids = append(kubeVirtContainerSpecUids, containerSpecId)
				}
			}
			...
			if err != nil {
				glog.Warningf("Could not create commoditiesSold for container[%s]: %v", name, err)
				continue
			}
			...
			commoditiesBought, err := builder.getCommoditiesBought(podId, name, containerMId,
				isCpuLimitSet, isMemLimitSet, isCpuRequestSet, isMemRequestSet)
			if err != nil {
				glog.Warningf("Could not create commoditiesBought for container[%s]: %v", name, err)
				continue
			}
			...
			//3. set properties
			properties := builder.getContainerProperties(pod, container, i)
			...
			// Disable action eligibility for Job/CronJob containers
			if util.IsJob(pod) {
				controllable = false
			}
	...
	return result, sidecars, kubeVirtPodUids, kubeVirtContainerSpecUids
}

func containerIsNotKubevirtVmContainer(container *api.Container) bool {
	return container.Name != pkgutil.ComputeContainerName
}
...
func (builder *containerDTOBuilder) isInjectedSidecarContainer(containerMId string) bool {
	...
	if err != nil {
		glog.Warningf("Could not get IsInjectedSidecar value for container %s: %v", containerMId, err)
...
func (builder *containerDTOBuilder) getCommoditiesSold(containerName, containerId, containerMId string,
	...
	for _, commSold := range cpuCommodities {
		if *commSold.Used > 100000 {
			glog.Errorf("High vCPU value %f for container '%s'.", *commSold.Used, containerName)
		}
	}
...
func (builder *containerDTOBuilder) getContainerProperties(pod *api.Pod, container *api.Container, index int) []*proto.EntityDTO_EntityProperty {
	properties := property.AddContainerProperties(builder.clusterSummary.Name, pod.Namespace, pod.Name, container.Name, index)
