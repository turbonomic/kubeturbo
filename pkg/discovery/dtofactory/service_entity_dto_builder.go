...
func (builder *ServiceEntityDTOBuilder) BuildDTOs() []*proto.EntityDTO {
	...
	for service, podList := range builder.ClusterSummary.Services {
		...
		if err := builder.createCommoditySold(ebuilder, pods, serviceName); err != nil {
			glog.Warningf("Could not create commodity sold for service %s: %v", serviceName, err)
			continue
		}
		...
		// set the ip property, service UID, kube-system namespace UID, service namespace & service name property for stitching
		ebuilder.WithProperty(getIPProperty(pods, svcUID)).WithProperty(getUUIDProperty(id)).
			...
			WithProperty(property.BuildNamespaceProperty(service.Namespace)).
			WithProperty(property.BuildServiceNameProperty(service.Name)).
			WithProperty(property.BuildFullyQualifiedNameProperty(strings.Join([]string{
				builder.ClusterSummary.Name, service.Namespace, service.Name}, commonutil.NamingQualifierSeparator))).
			WithProperties(property.BuildServiceLabelProperties(service))
