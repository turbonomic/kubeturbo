...
func isOperatorControlled(controller *repository.K8sController) bool {
	...
	// If the controller is configured in the workload and/or namespace exclusion lists, we do NOT consider it
	// operator-controlled.
...
func (builder *containerSpecDTOBuilder) BuildDTOs() ([]*proto.EntityDTO, []string, []string) {
	var result []*proto.EntityDTO
	var operatorControlledContainerSpecs, systemNameSpaceContainerSpecs []string
	for containerSpecId, containerSpec := range builder.containerSpecMetricsMap {
		...
		if builder.clusterSummary != nil {
			...
			if found {
				...
				if isOperatorControlled(controller) {
	...
	return result, operatorControlledContainerSpecs, systemNameSpaceContainerSpecs
}
