...
func (builder *groupDTOBuilder) createGroup(entityGroup *repository.EntityGroup, entityType metrics.DiscoveredEntityType,
	...
	if entityGroup.ParentKind == "StatefulSet" && protoType == proto.EntityDTO_CONTAINER_POD {
		...
		settingsPolicy, err := group.NewSettingPolicyBuilder().
			WithDisplayName(fmt.Sprintf("StatefulSet Pods Move Recommend Only [%s]", builder.targetId)).
			WithName(fmt.Sprintf("StatefulSet Pods-%s", builder.targetId)).
		...
		groupBuilder.WithSettingPolicy(settingsPolicy)
	}
