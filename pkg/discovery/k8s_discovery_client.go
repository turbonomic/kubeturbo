...
func (dc *K8sDiscoveryClient) DiscoverWithNewFramework(targetID string) ([]*proto.EntityDTO, []*proto.GroupDTO, error) {
			...
			if err != nil {
				...
				if err != nil {
					...
					nodesPods, podsWithAffinities, hostnameSpreadWorkloads, otherSpreadPods, otherSpreadWorkloads,
						podNonHostnameAntiTermTopologyKeys, affinityMapper = affinityProcessor.ProcessAffinities(clusterSummary.Pods)
				}
	...
	taskCount := dc.dispatcher.Dispatch(nodes, nodesPods, podsWithAffinities, otherSpreadPods, hostnameSpreadWorkloads, clusterSummary,
		node2nodegroup, podNonHostnameAntiTermTopologyKeys, affinityMapper)
	...
	containerSpecDtos, operatorControlledContainerSpecs, systemNameSpaceContainerSpecs := containerSpecDiscoveryWorker.Do(clusterSummary,
	...
	groupDTOs, _ := entityGroupDiscoveryWorker.Do(result.EntityGroups, result.SidecarContainerSpecs,
		operatorControlledContainerSpecs, systemNameSpaceContainerSpecs, result.PodsWithVolumes, result.NotReadyNodes, result.MirrorPodUids)
