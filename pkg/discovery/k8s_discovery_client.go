...
func NewK8sDiscoveryClient(config *DiscoveryClientConfig) *K8sDiscoveryClient {
	...
	clusterProcessor := processor.NewClusterProcessor(k8sClusterScraper, config.probeConfig.NodeClient,
		config.ValidationWorkers, config.ValidationTimeoutSec, config.itemsPerListQuery, config.targetConfig.IsOpenShift())
...
func (dc *K8sDiscoveryClient) Discover(
	...
	if dc.Config.targetConfig.IsOpenShift() {
...
func (dc *K8sDiscoveryClient) DiscoverWithNewFramework(targetID string) ([]*proto.EntityDTO, []*proto.GroupDTO, error) {
		...
		if utilfeature.DefaultFeatureGate.Enabled(features.NewAffinityProcessing) {
			...
			if err != nil {
				...
				if err != nil {
					...
					nodesPods, podsWithAffinities, hostnameSpreadWorkloads, otherSpreadPods,
						otherSpreadWorkloads, podNonHostnameAntiTermTopologyKeys = affinityProcessor.ProcessAffinities(clusterSummary.Pods)
				}
	...
	if !utilfeature.DefaultFeatureGate.Enabled(features.IgnoreAffinities) {
		if utilfeature.DefaultFeatureGate.Enabled(features.NewAffinityProcessing) &&
			...
			nodeGrpDTOs, node2nodegroup = dtofactory.
				NewNodeGroupEntityDTOBuilder(clusterSummary, otherSpreadWorkloads,
					getOtherSpreadTopologyKeys(podNonHostnameAntiTermTopologyKeys)).BuildEntityDTOs()
	...
	taskCount := dc.dispatcher.Dispatch(nodes, nodesPods, podsWithAffinities, otherSpreadPods, hostnameSpreadWorkloads, clusterSummary,
		node2nodegroup, podNonHostnameAntiTermTopologyKeys)
	...
	containerSpecDtos, operatorControlledContainerSpecs := containerSpecDiscoveryWorker.Do(clusterSummary,
		result.ContainerSpecMetrics, dc.Config.containerUtilizationDataAggStrategy,
	...
	groupDTOs, _ := entityGroupDiscoveryWorker.Do(result.EntityGroups, result.SidecarContainerSpecs,
		operatorControlledContainerSpecs, result.PodsWithVolumes, result.NotReadyNodes, result.MirrorPodUids)
