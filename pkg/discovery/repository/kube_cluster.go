...
func (kc *K8sController) WithHasParent(hasParent bool) *K8sController {
	kc.HasParent = hasParent
	return kc
}

func (kc *K8sController) WithApiVersion(apiVersion string) *K8sController {
	kc.APIVersion = apiVersion
	return kc
}

func (kc *K8sController) HasSkipOperatorLabel() bool {
	label, exists := kc.Labels[commonUtil.SkipOperatorLabel]
	return exists && strings.EqualFold(label, "true")
}

func (kc *K8sController) HasOperatorExclusion() bool {
	return detectors.ExcludedOperatorControlledNamespace(kc.Namespace) ||
		detectors.ExcludedOperatorControlledWorkload(kc.Name, kc.Namespace)
}

func (kc *K8sController) Unstructured() *unstructured.Unstructured {
	unstructuredController := &unstructured.Unstructured{}
	unstructuredController.SetAPIVersion(kc.APIVersion)
	unstructuredController.SetKind(kc.Kind)
	unstructuredController.SetNamespace(kc.Namespace)
	unstructuredController.SetName(kc.Name)
	return unstructuredController
}

func (kc *K8sController) ObjectReference() v1.ObjectReference {
	objRef := v1.ObjectReference{
		Namespace: kc.Namespace,
		Name:      kc.Name,
	}
	objRef.SetGroupVersionKind(kc.Unstructured().GetObjectKind().GroupVersionKind())
	return objRef
}
...
func (kubeNamespace *KubeNamespace) ReconcileQuotas(quotas []*v1.ResourceQuota) {
	...
	for _, item := range quotas {
		...
		// Check for the annotation Kubeturbo adds when temporarily resizing a ResourceQuota to execute an action.
		// If this annotation exists, the discovered data should reflect the configured quota, NOT the temporarily resized quota.
		// This is so that the market doesn't think there's extra capacity to allow additional resize-up actions that cannot fit
		// within the actual quota limits.
		if annotationQuota, exists := item.Annotations[commonUtil.QuotaAnnotationKey]; exists {
			configuredQuota, err := commonUtil.DecodeQuota([]byte(annotationQuota))
			if err != nil {
				glog.Warningf("Error parsing configured ResourceQuota for Namespace %s from %s annotation. Returning current configuration.",
					item.Namespace, commonUtil.QuotaAnnotationKey)
			} else {
				glog.V(4).Infof("Discovered temporarily resized ResourceQuota for Namespace %s. Using original configuration in discovery data: %v",
					item.Namespace, configuredQuota)
				resourceHardList = configuredQuota.Spec.Hard
			}
		}
