...
// PodMetrics is a collection of allocation resources bought by a pod
...
// NamespaceMetrics is a collection of quota resources sold by a namespace entity
...
func (namespaceMetrics *NamespaceMetrics) AggregateUsed(partialUsed map[metrics.ResourceType][]metrics.Point) {
	...
	util.AccumulateMultiPoints(namespaceMetrics.Used, partialUsed)
}
...
type ContainerMetrics struct {
	Capacity []float64
	// Used could be []metrics.Point, [][]metrics.ThrottlingCumulative
	// [][]metrics.ThrottlingCumulative stores multi-points segregated per container.
	// Storing it this way helps in identifying metrics per container while aggregating
	// the metrics for containerSpecs.
	// If cumulative CPU usage data is collected for individual containers (i.e., []metrics.Cumulative),
	// they will be converted to []metrics.Point data type first before being assigned to Used.
...
func NewContainerSpecMetrics(namespace, controllerName, controllerUID, containerName, containerSpecId string) *ContainerSpecMetrics {
