...
func TestK8sRegistrationClient_GetActionMergePolicy(t *testing.T) {
	...
	for _, policy := range policies {
		...
		if sloSpec != nil {
			...
			assert.Equal(t, 6, len(commodityData))
			...
			assert.Contains(t, commodityTypes, proto.CommodityDTO_CONCURRENT_QUERIES)
			assert.Contains(t, commodityTypes, proto.CommodityDTO_SERVICE_TIME)
			assert.Contains(t, commodityTypes, proto.CommodityDTO_QUEUING_TIME)
			assert.Contains(t, commodityTypes, proto.CommodityDTO_LLM_CACHE)
		}
	}
