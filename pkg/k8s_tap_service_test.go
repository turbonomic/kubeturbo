...
func TestParseK8sTAPServiceSpecWithMissingTargetConfig(t *testing.T) {
	...
	config, err := ParseK8sTAPServiceSpec(configPath, defaultTargetName, nil, nil, CollectK8sTargetAndProbeInfoForTest)
	if err != nil {
		t.Fatalf("Error while parsing the spec file %s: %v", configPath, err)
	}

	// Check target config
	checkStartWith(config.ProbeCategory, "Container Platform", t)
...
func TestParseK8sTAPServiceSpecWithIncompleteCredential(t *testing.T) {
	...
	_, err := ParseK8sTAPServiceSpec(configPath, defaultTargetName, nil, nil, CollectK8sTargetAndProbeInfoForTest)
	if err == nil {
...
func CollectK8sTargetAndProbeInfoForTest(kubeConfig *rest.Config,
	kubeClient *kubernetes.Clientset, config *configs.K8sTargetConfig) {
	// NA
}

func CollectK8sTargetAndProbeInfoForTestOpenShift(kubeConfig *rest.Config,
	kubeClient *kubernetes.Clientset, config *configs.K8sTargetConfig) {
	config.IsOcp = true
}
