...
import (
	...
	capiclient "github.com/openshift/client-go/machine/clientset/versioned"
...
type ClusterScraperInterface interface {
	...
	GetMachineSetToNodesMap(nodes []*api.Node, isOpenShift bool) map[string][]*api.Node
...
func (s *ClusterScraper) GetMachineSetToNodesMap(allNodes []*api.Node, isOpenShift bool) map[string][]*api.Node {
	machineSetToNodes := make(map[string][]*api.Node)

	if !isOpenShift {
		glog.V(2).Infof("Skip MachineSet to Node mapping for non-OpenShift cluster")
		return machineSetToNodes
	}

	isEnabled, err := s.IsClusterAPIEnabled()
	if !isEnabled {
		glog.V(2).Infof("Skip MachineSet to Node mapping. %s", err)
		return machineSetToNodes
	}
...
func (s *ClusterScraper) UpdatePodControllerCache(
	...
	for _, pod := range pods {
		...
		if kind == commonutil.KindReplicationController || kind == commonutil.KindReplicaSet || kind == commonutil.KindJob {
...
//
//	, if its a "ReplicationController" its parent could be "DeploymentConfig" (as in openshift)
//
// , if its a "Job" its parent could be "CronJob".
...
func (s *ClusterScraper) GetPodControllerInfo(
	...
	if useCache {
		...
		if exists {
			...
			glog.Warningf("Could not get controller info cache data: controllerInfoCache is '%t' not 'OwnerInfo'",
				controllerInfoCache)
		}
	}
	...
	//2. if parent is "ReplicaSet", "ReplicationController" or "Job", check parent's parent
	...
	case commonutil.KindVirtualMachineInstance:
		res = schema.GroupVersionResource{
			Group:    commonutil.OpenShiftVirtualMachineInstanceGV.Group,
			Version:  commonutil.OpenShiftVirtualMachineInstanceGV.Version,
			Resource: commonutil.VirtualMachineInstanceResName}
		canHaveGrandParent = true
	default:
...
func (s *ClusterScraper) UpdateGitOpsConfigCache() {
	...
	if err != nil {
		glog.V(2).Infof("Could not discover GitOps configurations: %v", err)
		return
	}
	gitOpsConfigCache := make(map[string][]*gitopsv1alpha1.Configuration)
...
func (s *ClusterScraper) IsClusterAPIEnabled() (bool, error) {
	if s.CApiClient == nil {
		return false, fmt.Errorf("Cluster API client is nil")
	}
	serviceString := fmt.Sprintf("Cluster API service \"%s\"", clusterAPIGroupVersion)
	...
	if err != nil {
		return false, fmt.Errorf("%s is not available: %v", serviceString, err)
	}
	return true, nil
}
