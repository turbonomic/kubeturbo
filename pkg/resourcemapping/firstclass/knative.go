...
// the actual ownerreference is a long chain, in order to save the access to api server, let's just extract name from deployment name by removing everything after "component name"
...
func (k *KNativeServiceOwner) replace(trueRef metav1.OwnerReference, owned *unstructured.Unstructured, ownedResourcePaths []string) (map[*unstructured.Unstructured][]string, error) {
	...
	for _, path := range ownedResourcePaths {
		...
		case deploymentReplicasPath:
			...
			if err != nil || !exists {
				glog.Errorf("failed to locate the replicas field in controller object. exists: %t, err: %s. Obj:%v", exists, err.Error(), owned)
				continue
			}
