...
func (pc *parentController) update(updatedSpec *k8sControllerSpec) error {
	...
	if !pc.shouldSkipOperator(pc.obj) && isOwnerSet {
		...
		objMap, err := pc.ormClient.CheckAndReplaceWithFirstClassOwners(pc.obj.DeepCopy(), ownedResourcePaths)
		if err != nil {
			return fmt.Errorf("error in owner checking, err: %s", err.Error())
