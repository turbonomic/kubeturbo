...
import (
	...
	"github.ibm.com/turbonomic/kubeturbo/pkg/action/executor/custom"
	"github.ibm.com/turbonomic/kubeturbo/pkg/action/executor/firstclass"
...
const (
	...
	KubevirtVirtualMachineRightSizePathTemplate pathTemplate = ".spec.template.spec.domain.resources"
)
...
var (
	controllerTypeToPathTemplate = map[string]actionTypeToPathTemplate{
		...
		util.KindVirtualMachine: {
			proto.ActionItemDTO_RIGHT_SIZE: KubevirtVirtualMachineRightSizePathTemplate,
		},
	}
)
...
func (pc *parentController) get(name string) (*k8sControllerSpec, error) {
	...
	replicas := int64(0)
	found := false
	if typesDoNotHaveReplicaField(kind) {
	...
	podSpec := apicorev1.PodSpec{}
	updated, err := custom.GeneratePodSpecForCustomController(kind, obj.Object, &podSpec)
	if err != nil {
		return nil, fmt.Errorf("error retrieving podSpec for custom controller from %s %s: %v", kind, objName, err)
	}
	if !updated {
...
func typesDoNotHaveReplicaField(kind string) bool {
	return kind != util.KindDaemonSet && kind != util.KindVirtualMachine
}
...
func (pc *parentController) update(updatedSpec *k8sControllerSpec) error {
	...
	var updated bool
	pc.obj, updated, err = custom.CheckAndUpdateCustomController(kind, pc.obj, updatedSpec.podSpec)
	if err != nil {
		return err
	}
	if !updated {
	...
	if !pc.shouldSkipOperator(pc.obj) && isOwnerSet {
		...
		// check and replace the object with first class owner object, no changes to obj if there is no first class owner
		// then use new obj for orm to check/update
		objMap := firstclass.CheckAndReplaceWithFirstClassOwners(pc.obj.DeepCopy(), ownedResourcePaths)

		for obj, paths := range objMap {

			// TODO: update the owner info to firstclass object. do we really need it? There won't be any v1 orm for first class citizen.
			ownerResources, err := pc.ormClient.GetOwnerResourcesForOwnedResources(obj, ownerInfo, paths)
			if err != nil {
				return fmt.Errorf("unable to get owner resources: %v", err)
			}
			glog.V(2).Infof("Finish top level owner resources search for obj: %v, ownerResources: %v", obj, ownerResources)
			err = pc.ormClient.UpdateOwners(obj, ownerInfo, ownerResources)
			if err != nil {
				return err
			}
		}
		...
		err = custom.CompleteCustomController(pc.clients.dynNamespacedClient, kind, pc.obj)
		if err != nil {
			return err
		}

		glog.Infof("Updated workload controller %s/%s", pc.obj.GetNamespace(), pc.obj.GetName())
		...
		if kind != util.KindVirtualMachine && utilfeature.DefaultFeatureGate.Enabled(features.AllowIncreaseNsQuota4Resizing) {
...
func (pc *parentController) revert() error {
	...
	if err = custom.CompleteCustomController(pc.clients.dynNamespacedClient, pc.obj.GetKind(), pc.obj); err != nil {
		return err
	}
	return nil
}
...
func (pc *parentController) shouldSkipOperator(controller *unstructured.Unstructured) bool {
	...
	labelVal, exists := labels[util.SkipOperatorLabel]
	if exists && strings.EqualFold(labelVal, "true") {
		glog.Infof("Directly updating '%s %s/%s' regardless of Operator because '%s' label is set to true.",
			controller.GetKind(), controller.GetNamespace(), controller.GetName(), util.SkipOperatorLabel)
		return true
	}
	return false
}
...
func (pc *parentController) getOwnedResourcePaths(controllerType string, k8sSpec *k8sControllerSpec) ([]string, error) {
	...
	if pc.actionType == proto.ActionItemDTO_RIGHT_SIZE && pc.name != util.KindVirtualMachine {
