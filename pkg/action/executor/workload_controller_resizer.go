...
func NewWorkloadControllerResizer(ae TurboK8sActionExecutor, kubeletClient *kubeclient.KubeletClient,
	sccAllowedSet map[string]struct{}, lockMap *actionutil.ExpirationMap,
) *WorkloadControllerResizer {
...
func (r *WorkloadControllerResizer) Execute(input *TurboActionExecutorInput) (*TurboActionExecutorOutput, error) {
	...
	for _, item := range actionItems {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("could not find container %v in parents pod template spec. It is likely an injected sidecar", item.GetCurrentSE().GetDisplayName())
	...
	hasLimitRange := false
	limitRanges, err := r.clusterScraper.Clientset.CoreV1().LimitRanges(namespace).List(context.TODO(), metav1.ListOptions{})
	if err == nil && len(limitRanges.Items) > 0 {
		hasLimitRange = true
	}
	desiredPod := buildDesiredPod4QuotaEvaluation(namespace, resizeSpecs, *podSpec, hasLimitRange)
	if hasLimitRange {
		...
		if limitrangeViolateErr != nil {
			...
			return nil, fmt.Errorf("limitrange violation:%v", limitrangeViolateErr)
		}
	}
...
func (r *WorkloadControllerResizer) ExecuteList(input *TurboActionExecutorInput) (*TurboActionExecutorOutput, error) {
	return nil, fmt.Errorf("batched action execution is not supported for workload controller resize")
}
...
func getWorkloadControllerInfo(targetSE *proto.EntityDTO) (string, string, string, error) {
	...
	case *proto.EntityDTO_WorkloadControllerData_CustomControllerData:
		...
		if kind != util.KindDeploymentConfig && kind != util.KindVirtualMachine {
	...
	namespace, err := property.GetNamespaceFromProperties(targetSE.GetEntityProperties())
	if err != nil {
		return "", "", "", err
	}
...
func (r *WorkloadControllerResizer) getWorkloadControllerSpec(parentKind, namespace, name string) (*k8sapi.PodSpec, int64, bool, error) {
	...
	updated, err := custom.GeneratePodSpecForCustomController(parentKind, obj.Object, &podSpec)
	if err != nil {
		return nil, 0, false, fmt.Errorf("error retrieving vm resources from %s %s/%s: %v", objKind, namespace, name, err)
	}
	if !updated {
	...
	} else if parentKind != util.KindVirtualMachine {
...
func resizeWorkloadController(clusterScraper *cluster.ClusterScraper, ormClient *resourcemapping.ORMClientManager,
	...
	err = controllerUpdater.updateWithRetry(&controllerSpec{resizeSpecs: specs})
	if err != nil {
...
func buildDesiredPod4QuotaEvaluation(namespace string, resizeSpecs []*containerResizeSpec, podSpec k8sapi.PodSpec, hasLimitRanges bool) *k8sapi.Pod {
	for _, spec := range resizeSpecs {
		...
		// Sanitize the resource requests and limits
		if hasLimitRanges {
			actionutil.SanitizeResources(&podSpec)
		}
	}
	return &k8sapi.Pod{
