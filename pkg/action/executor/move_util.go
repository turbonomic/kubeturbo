...
func addAdoptionLabels(labelsFromParent, labelsFromPod map[string]string) map[string]string {
	// We pick only these keys from the old pod labels to ensure it is adopted by its
	// parent properly.
	adoptionLabelKeys := []string{
	...
	for key, val := range labelsFromPod {
		if keyInKeys(key, adoptionLabelKeys) {
			labelsFromParent[key] = val
		}
	}
	return labelsFromParent
}
...
func movePod(clusterScraper *cluster.ClusterScraper, pod *api.Pod, nodeName, parentKind, parentName string,
	...
	npod, labelsFromParent, err := createClonePod(clusterScraper.Clientset, pod, parentForPodSpec, updateQuotaToAllowMoves, nodeName)
	if err != nil {
	...
	// Add autogenerated adoption labels to the labels from parent from among the labels
	// that we picked from the original pod. We discard rest of the labels which might be added
	// by other controllers (eg istio) expecting they will be added again on the new pod.
	xpodLabels := addAdoptionLabels(labelsFromParent, podLabels)

	//TODO: compare resourceVersion of xpod and npod before updating
	if (xpodLabels != nil) && len(xpodLabels) > 0 {
...
func createClonePod(client *kclient.Clientset, pod *api.Pod,
	parent *unstructured.Unstructured, updateQuotaToAllowMoves bool, nodeName string) (*api.Pod, map[string]string, error) {
	npod := &api.Pod{}

	labelsFromParent := make(map[string]string)
	...
	if parent != nil {
		...
		if err != nil || !found {
			return nil, labelsFromParent, fmt.Errorf("movePod: error retrieving podSpec from %s %s: %v", kind, parentName, err)
		}

		podSpec := api.PodSpec{}
		if err := runtime.DefaultUnstructuredConverter.FromUnstructured(podSpecUnstructured.(map[string]interface{}), &podSpec); err != nil {
			return nil, labelsFromParent, fmt.Errorf("movePod: error converting unstructured pod spec to typed pod spec for %s %s: %v", kind, parentName, err)
		}
		...
		if err != nil {
			return nil, labelsFromParent, fmt.Errorf("movePod: error retrieving annotations from %s %s: %v", kind, parentName, err)
		}
		if found {
			if err := runtime.DefaultUnstructuredConverter.FromUnstructured(annotationsUnstructured.(map[string]interface{}), &annotations); err != nil {
				return nil, labelsFromParent, fmt.Errorf("movePod: error converting unstructured annotations to typed annotations for %s %s: %v", kind, parentName, err)
			}
		}

		// labels also from parent
		labelsUnstructured, found, err := unstructured.NestedFieldCopy(parent.Object, "spec", "template", "metadata", "labels")
		if err != nil {
			return nil, labelsFromParent, fmt.Errorf("movePod: error retrieving labels from %s %s: %v", kind, parentName, err)
		}
		if found {
			if err := runtime.DefaultUnstructuredConverter.FromUnstructured(labelsUnstructured.(map[string]interface{}), &labelsFromParent); err != nil {
				return nil, labelsFromParent, fmt.Errorf("movePod: error converting unstructured labels to typed labels for %s %s: %v", kind, parentName, err)
			}
		}

		// The podspec and annotations are set here before calling copyPodWithoutLabel below
		// For a bare pod both (annotations and labels) are set from the original pod
		npod.Spec = podSpec
		npod.Annotations = annotations
		copySpec = false
	}

	copyPodWithoutLabel(pod, npod, copySpec)
