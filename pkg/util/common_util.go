...
const (
	...
	// NamingQualifierSeparator is used in an entity's fully qualified name to separate each qualifier from the name
	// of the entity, e.g., a fully qualified service name is: <cluster_name>/<namespace_name>/<service_name>
	NamingQualifierSeparator = "/"
...
func RetryDuring(attempts int, timeout time.Duration, sleep time.Duration, myfunc func() error) error {
	...
	for i := 0; ; i++ {
		...
		if timeout > 0 {
			if delta := time.Now().Sub(t0); delta > timeout {
				err = fmt.Errorf("failed after %d attempts (during %v) last error: %v", i+1, delta, err)
				return err
			}
		}
	...
	err = fmt.Errorf("failed after %d attempts, last error: %v", attempts, err)
	return err
}
...
func RetrySimple(attempts int32, timeout, sleep time.Duration, myfunc func() (bool, error)) error {
	...
	for i = 0; ; i++ {
		...
		if timeout > 0 {
			if delta := time.Now().Sub(t0); delta > timeout {
				glog.Errorf("Failed after %d attempts (during %v) last error: %v", i+1, delta, err)
				return err
			}
		}
	...
	glog.Errorf("Failed after %d attempts, last error: %v", attempts, err)
	return err
}
...
func GetKubeturboNamespace() string {
	...
	if namespace == "" {
		glog.Warningf("OS environment variable %s is not set. Using default namespace: '%s'", kubeturboNSEnvVar, DefaultNamespace)
...
func ServiceAccountNameForSCC(sccName string) string {
	return KubeturboSCCPrefix + sccName
}

func GetServiceAccountForSCC(sccName string) *corev1.ServiceAccount {
	return &corev1.ServiceAccount{
		ObjectMeta: metav1.ObjectMeta{
			Name: ServiceAccountNameForSCC(sccName),
		},
	}
}
